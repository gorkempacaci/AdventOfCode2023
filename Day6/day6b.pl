...
main :-
  ...
  foldl([X,Y,Z]>>(atom_concat(Y,X,Z)), Times, '', TimesStr),
  atom_number(TimesStr, Time),
  foldl([X,Y,Z]>>(atom_concat(Y,X,Z)), Distances, '', DistancesStr),
  atom_number(DistancesStr, Distance),
